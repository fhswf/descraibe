/* Isolated server: work/two_stage_browser_server.py; deterministic models, real API/SSE/video. */
...
(async () => {
  ...
  try {
    ...
    await expect(page.getByText('1 Face-Änderungen vorgemerkt · 0 Split-Änderungen vorgemerkt', { exact: true })).toBeVisible();
    await page.getByRole('button', { name: 'Review-Änderungen speichern', exact: true }).click();
    await expect(page.getByText(/Personen & Cluster ist veraltet/)).toBeVisible();
    assert.deepEqual((await snapshot()).persons, before.persons);
    await page.reload(); await nav(5);
    ...
    await nav(6);
    await expect(page.getByText(/Veraltet: Gesichtsbereinigung/)).toBeVisible();
    await page.getByRole('button', { name: /Personen & Cluster ausführen/ }).click();
    await expect.poll(async () => (await snapshot()).identities_stale).toBe(false);
    ...
    console.log('PASS: quality-only review, dynamic bbox at two viewport sizes, exclude/reload/stale/rerun, unchanged persons until rerun');
    ...
    await expect(page.getByRole('article', { name: 'Person 2', exact: true })).toContainText('Manuell geprüft');
    await page.getByRole('button', { name: /Nicht zugeordnete Tracks/ }).click();
    ...
    // Split and full undo are staged in the same review and stale completed identities.
    await track(1).getByRole('button', { name: 'Vor Frame 31 teilen', exact: true }).click();
    await expect(page.getByText(/Vorgemerkt: Schnitt vor Frame 31/)).toBeVisible();
    ...
    assert.deepEqual((await snapshot()).persons[0].track_ids, [6, 7]);
    assert.equal((await snapshot()).identities_stale, true);
    console.log('PASS: split selection, two disjoint segments, clustering uses segments, reversible split leaves completed persons frozen');
    ...
    console.log('PASS: whole-track batch, unassigned/new/assign, example/name/ID picker, metadata/reload, merge/delete, face restore leaves assignments untouched');
