...
function AttributesPanel({ jobId }: { jobId: string }) {
    ...
    const save = async () => {
        ...
        try {
            ...
            setSnapshot(data); setPending({}); void fetchJobData(jobId, true);
        ...
        {open && snapshot && <div role="dialog" aria-modal="true" aria-label="Attribut-Review" className="fixed inset-0 z-[60] bg-black/60 flex items-center justify-center p-2 sm:p-4"
            ...
            <div className="w-[97vw] h-[94vh] bg-bg-surface rounded-xl border border-border-subtle shadow-2xl flex flex-col overflow-hidden">
                ...
                <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-6">
                    ...
                    <div><h4 className="text-lg font-semibold">{person.name || `Person ${person.person_id}`} · ID {person.person_id}</h4><p className="text-xs text-text-secondary">{statuses[person.status] || person.status}</p></div>
                ...
                <div className="p-4 border-t border-border-subtle bg-bg-card shrink-0">
                    ...
                    <div className="flex flex-wrap gap-3 items-center justify-between">
                        <div className="flex flex-wrap gap-3 items-center">
                            <span className="text-xs text-text-secondary" aria-live="polite">{Object.keys(pending).length} Attributänderungen vorgemerkt</span>
                            <button disabled={saving || !dirty} onClick={() => { if (window.confirm('Ungespeicherte Attributänderungen verwerfen?')) { setPending({}); setRefresh(n => n+1); } }} className="text-xs underline">Verwerfen / Neu laden</button>
...
export function StepAttributes() {
    ...
    if (currentStep !== STEP.attributes) return null;
